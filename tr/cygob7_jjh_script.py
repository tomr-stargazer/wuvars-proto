...
watso = atpy.Table(path+"Watso_ALLDATA_cleaned_errorcorrected.fits")
...
data = atpy.Table(path+"cleaned_errorcorrected_data.fits")

# special data file for 2 super faint wise_extras that were missed in the
# previous passes
welo = atpy.Table(path+"WISE_extras_lonelytwo_only_ALLDATA.fits.gz")
...
for s, n in zip(wise_disks[rwd], wise_disks_names[rwd]):
    fig = plot3.jjh(watso, s, name=str(n), color_slope=True, date_offset=54579,
...
for s, n in zip(wise_extras[rwe], wise_extras_names[rwe]):
    fig = plot3.jjh(data, s, name=str(n), color_slope=True, date_offset=54579,
